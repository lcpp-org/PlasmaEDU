...
Et = np.zeros((inp.Nx,len(inp.time_vector)))
Et_norm = np.zeros((len(inp.time_vector)))
for i in range(inp.Nx):
    Et[i,:] = 4.0*0.001 * 0.3677 * np.exp(-0.1533*inp.time_vector)*np.sin(0.5*inp.X[i])*np.cos(1.4156*inp.time_vector - 0.5326245)

for n in range(len(inp.time_vector)):
    Et_norm[n] = np.sqrt(np.trapz(Et[:,n]*Et[:,n],x=inp.X))

### DAMPING RATE FITTING
# Take derivative of Enorm
dedx = np.zeros((len(Enorm),1))
for i in range(1,len(Enorm)-1):
    dedx[i] = -( Enorm[i+1]  -   Enorm[i-1])             / 2.0 / inp.dt
dedx[0]     =  ( 3*Enorm[0]  - 4*Enorm[1]  + Enorm[2]  ) / 2.0 / inp.dt
dedx[-1]    = -( 3*Enorm[-1] - 4*Enorm[-2] + Enorm[-3] ) / 2.0 / inp.dt
...
for i in range(len(dedx_0)):
    ...
    x_max[i] = inp.time_vector[dedx_0[i]]

    # y values (maxima)
    Enorm_max[i] = Enorm[dedx_0[i]]

# Theoretical Damping Rate (for linear Landau damping)
imw = np.sqrt(np.pi/8.0) * (1.0/(inp.k**3.0)) * np.exp(-1.0/(2.0*(inp.k**2.0)) - (3.0/2.0))
...
plt.semilogy(inp.time_vector,Enorm,linewidth=2)
